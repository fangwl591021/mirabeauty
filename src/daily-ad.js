...
async function activeCampaign(db) {
  return db.prepare(`
    SELECT id, name, required_creative_count FROM ad_campaigns
...
export async function getDailyAdCampaign(db, userId) {
  ...
  const creativeResult = await db.prepare(`
    SELECT id, creative_type, title, media_url, preview_url, target_url, required_watch_seconds, required_completion_ratio, display_order
  ...
  `).bind(userId, campaign.id, date).first();
  return {
    campaign: { id: campaign.id, name: campaign.name, requiredCreativeCount: campaign.required_creative_count },
...
export async function startAdView(db, userId, creativeId) {
  ...
  `).bind(creativeId, campaign.id).first();
  ...
  `).bind(sessionId, await sha256(token), userId, campaign.id, creative.id).run();
  return { ok: true, token, expiresIn: 600 };
}

export async function recordAdViewProgress(db, { userId, token, watchedSeconds, completionRatio, pageVisible = true, now = Date.now() }) {
  ...
  `).bind(await sha256(String(token || '')), userId).first();
  if (!session || Date.parse(session.expires_at) < now) return { ok: false, reason: 'view_session_expired' };
  const elapsedSeconds = Math.max(0, Math.floor((now - Date.parse(session.started_at)) / 1000));
  const reportedSeconds = Math.max(0, Math.floor(Number(watchedSeconds) || 0));
  const nextSeconds = pageVisible ? Math.max(Number(session.observed_seconds), Math.min(reportedSeconds, elapsedSeconds)) : Number(session.observed_seconds);
  const nextRatio = pageVisible ? Math.max(Number(session.completion_ratio), Math.min(1, Math.max(0, Number(completionRatio) || 0))) : Number(session.completion_ratio);
  const qualified = nextSeconds >= Number(session.required_watch_seconds) && nextRatio >= Number(session.required_completion_ratio);
  await db.prepare(`
    UPDATE ad_view_sessions SET observed_seconds = ?, completion_ratio = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?
  `).bind(nextSeconds, nextRatio, qualified ? 'qualified' : 'active', session.id).run();
  if (qualified) {
    ...
    `).bind(newId('adviewevent'), userId, session.campaign_id, session.creative_id, date, session.id, nextSeconds, nextRatio).run();
  }
  return { ok: true, qualified, observedSeconds: nextSeconds, completionRatio: nextRatio };
}
...
export async function checkInDailyAd(db, userId) {
  ...
  if (!campaign) return { ok: false, reason: 'no_active_campaign' };
  const date = businessDate();
  const qualifying = await db.prepare(`
    SELECT COUNT(*) AS count FROM daily_ad_view_events
    WHERE platform_user_id = ? AND campaign_id = ? AND business_date = ? AND qualified_at IS NOT NULL
  `).bind(userId, campaign.id, date).first();
  if (Number(qualifying?.count || 0) < Number(campaign.required_creative_count)) {
    return { ok: false, reason: 'watch_requirement_not_met', qualifiedCreativeCount: Number(qualifying?.count || 0) };
  }
  const checkinId = newId('dailycheckin');
  try {
    await db.prepare(`INSERT INTO daily_checkins (id, platform_user_id, campaign_id, business_date) VALUES (?, ?, ?, ?)`)
      .bind(checkinId, userId, campaign.id, date).run();
  } catch (error) {
    if (String(error.message || '').includes('UNIQUE constraint failed: daily_checkins.platform_user_id, daily_checkins.campaign_id, daily_checkins.business_date')) {
