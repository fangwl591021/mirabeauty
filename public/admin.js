...
const defaultTemplate = () => ({active:true,keywords:["簽到贈點活動"],altText:"簽到贈點活動",rotationMode:"random",pages:[defaultPage()]});
function normalizeTemplate(data = {}) { const base=defaultTemplate(); return { active:data.active !== false, keywords:Array.isArray(data.keywords)&&data.keywords.length?data.keywords.map(x=>String(x).trim()).filter(Boolean):base.keywords, altText:String(data.altText || base.altText), rotationMode:validRotation(data.rotationMode), pages:(Array.isArray(data.pages)&&data.pages.length?data.pages:base.pages).slice(0,12).map(p=>({imageUrl:String(p.imageUrl||""),imageLink:String(p.imageLink||""),bubbleSize:validSize(p.bubbleSize),imageAspectRatio:validRatio(p.imageAspectRatio),imageAspectMode:validMode(p.imageAspectMode),buttons:(Array.isArray(p.buttons)&&p.buttons.length?p.buttons:[defaultButton()]).slice(0,4).map(b=>({label:String(b.label||"按鈕"),type:b.type==="uri"?"uri":"message",text:String(b.text||""),uri:String(b.uri||""),color:String(b.color||"")}))})) }; }
...
async function saveCheckinTemplate(){const t=collectTemplate();if(!t.keywords.length||!t.pages.length||t.pages.some(p=>!p.imageUrl))return templateStatus("請填寫關鍵字，並為每一頁上傳圖片或貼上 HTTPS 圖片網址。",false);const button=$("#templateSave");button.disabled=true;button.textContent="儲存中...";try{const data=await api("/v1/admin/checkin-template",t);renderCheckinTemplate(data.template||t);templateStatus("已儲存模板。",true)}catch(error){templateStatus(error.message,false)}finally{button.disabled=false;button.textContent="儲存模板"}}
