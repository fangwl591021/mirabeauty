...
function calendarEntryUrl(sessionId) { return fetch('/api/config').then(r => r.json()).then(config => config.liffId ? `https://liff.line.me/${config.liffId}?courseSession=${encodeURIComponent(sessionId)}` : `${location.origin}/?courseSession=${encodeURIComponent(sessionId)}`); }
...
function renderCalendarList() { const node = $("#calendarList"); node.innerHTML = calendarEvents.length ? calendarEvents.map(event => `<article class="calendar-list-item"><div><strong>${esc(event.title || event.courseTitle)}</strong><p>${esc(event.courseTitle)}｜${esc(calendarRange(event))}</p><small>${event.mode === "physical" ? esc(event.venueName || event.venueAddress || "現場") : "線上"}｜報名／簽到 ${esc(calendarTimeOnly(event.checkinOpensAt))}–${esc(calendarTimeOnly(event.checkinClosesAt))}</small></div><div><button class="outline" data-calendar-edit="${esc(event.sessionId)}">編輯</button><button class="outline" data-calendar-qr="${esc(event.sessionId)}">活動 QR</button></div></article>`).join("") : '<p class="muted">目前沒有行事曆活動。請新增場次。</p>'; document.querySelectorAll("[data-calendar-edit]").forEach(button => button.onclick = () => openCalendarEditor(button.dataset.calendarEdit)); document.querySelectorAll("[data-calendar-qr]").forEach(button => button.onclick = () => showCalendarQr(button.dataset.calendarQr)); }
async function loadCalendar() { try { const [courses, events] = await Promise.all([api('/v1/admin/courses'), api('/v1/admin/calendar/events')]); calendarCourses = courses.courses || []; calendarEvents = events.events || []; renderCalendarCourses(); renderCalendarMonth(); renderCalendarList(); } catch (error) { showStatus(error.message, 'error'); } }
function clearCalendarEditor() { $("#calendarEditor").hidden = true; $("#calendarEventId").value = ""; $("#calendarQr").classList.add("hidden"); calendarStatus(""); }
function openCalendarEditor(id = '') { const event = calendarEvents.find(row => row.sessionId === id); $("#calendarEditor").hidden = false; $("#calendarEditorTitle").textContent = event ? "編輯活動" : "新增活動"; $("#calendarEventId").value = event?.sessionId || ""; $("#calendarCourseId").value = event?.courseId || ""; $("#calendarSessionTitle").value = event?.title || ""; $("#calendarMode").value = event?.mode || "physical"; $("#calendarEventDate").value = event ? calendarDateOnly(event.startsAt) : ""; $("#calendarStartsAt").value = event ? calendarTimeOnly(event.startsAt) : ""; $("#calendarEndsAt").value = event ? calendarTimeOnly(event.endsAt) : ""; $("#calendarRegistrationStartsAt").value = event ? calendarTimeOnly(event.checkinOpensAt) : ""; $("#calendarRegistrationEndsAt").value = event ? calendarTimeOnly(event.checkinClosesAt) : ""; $("#calendarVenueName").value = event?.venueName || ""; $("#calendarVenueAddress").value = event?.venueAddress || ""; $("#calendarMeetingUrl").value = event?.meetingUrl || ""; $("#calendarCheckinCode").value = ""; $("#calendarDelete").hidden = !event; calendarStatus(event ? "已載入活動。簽到碼留空即可保留原有設定。" : "填完場次與報名時間後儲存。", false); $("#calendarEditor").scrollIntoView({behavior:'smooth', block:'start'}); }
async function showCalendarQr(sessionId) { const url = await calendarEntryUrl(sessionId); $("#calendarQr").classList.remove("hidden"); const holder = $("#calendarQrImage"); holder.innerHTML = ""; if (window.QRCode) new QRCode(holder, {text:url, width:148, height:148}); else holder.textContent = "QR 載入失敗"; $("#calendarQrUrl").value = url; $("#calendarCopyQr").onclick = async () => { await navigator.clipboard.writeText(url); showStatus("活動網址已複製"); }; $("#calendarQr").scrollIntoView({behavior:'smooth',block:'nearest'}); }
async function saveCalendarEvent() { const date = $("#calendarEventDate").value; const payload = { id:$("#calendarEventId").value, courseId:$("#calendarCourseId").value, title:$("#calendarSessionTitle").value.trim(), mode:$("#calendarMode").value, startsAt:calendarDateTime(date,$("#calendarStartsAt").value), endsAt:calendarDateTime(date,$("#calendarEndsAt").value), checkinOpensAt:calendarDateTime(date,$("#calendarRegistrationStartsAt").value), checkinClosesAt:calendarDateTime(date,$("#calendarRegistrationEndsAt").value), venueName:$("#calendarVenueName").value.trim(), venueAddress:$("#calendarVenueAddress").value.trim(), meetingUrl:$("#calendarMeetingUrl").value.trim(), checkinCode:$("#calendarCheckinCode").value.trim(), status:'scheduled' }; try { const result = await api('/v1/admin/calendar/events', payload); calendarStatus("已儲存。課程掃碼報名會依「課程報名」規則贈點。"); await loadCalendar(); openCalendarEditor(result.id); } catch(error) { calendarStatus(error.message, true); } }
