...
const defaultTemplate = () => ({active:true,entryUrl:"",altText:"簽到贈點活動",rotationMode:"random",pages:[defaultPage()]});
function normalizeTemplate(data = {}) { const base=defaultTemplate(); return { active:data.active !== false, entryUrl:String(data.entryUrl || ""), altText:String(data.altText || base.altText), rotationMode:validRotation(data.rotationMode), pages:(Array.isArray(data.pages)&&data.pages.length?data.pages:base.pages).slice(0,12).map(p=>({imageUrl:String(p.imageUrl||""),imageLink:String(p.imageLink||""),bubbleSize:validSize(p.bubbleSize),imageAspectRatio:validRatio(p.imageAspectRatio),imageAspectMode:validMode(p.imageAspectMode),buttons:(Array.isArray(p.buttons)&&p.buttons.length?p.buttons:[defaultButton()]).slice(0,4).map(b=>({label:String(b.label||"按鈕"),type:b.type==="uri"?"uri":"message",text:String(b.text||""),uri:String(b.uri||""),color:String(b.color||"")}))})) }; }
...
function renderCheckinTemplate(template) { const t=normalizeTemplate(template); checkinTemplateDraft=t; $("#templateActive").checked=t.active; $("#templateEntryUrl").value=t.entryUrl; $("#templateAltText").value=t.altText; $("#templateRotationMode").value=t.rotationMode; $("#templatePages").innerHTML=t.pages.map(pageHtml).join(""); refreshTemplatePreview(); }
