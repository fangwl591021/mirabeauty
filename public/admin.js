...
async function geocodeCalendarAddress() { const address = $("#calendarVenueAddress").value.trim(); const status = $("#calendarGeocodeStatus"); if (!address) { status.textContent = "請先輸入場地地址"; return; } status.textContent = "Google 地圖定位中…"; try { const result = await api('/v1/admin/geocode', { address }); $("#calendarVenueLatitude").value = result.latitude; $("#calendarVenueLongitude").value = result.longitude; $("#calendarVenueAddress").value = result.formattedAddress || address; status.textContent = "已取得場地座標"; } catch (error) { status.textContent = error.message === 'GOOGLE_MAPS_API_KEY is not configured' ? '尚未設定 Google Maps API Key' : '找不到此地址，請確認後再試'; } }
...
$("#calendarGeocode")?.addEventListener('click', geocodeCalendarAddress);
