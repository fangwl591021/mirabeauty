...
const defaultButton = (type = "uri") => ({ label:type === "uri" ? "開啟連結" : "簽到贈點", type, text:type === "uri" ? "" : "會員打卡", uri:"", color:"" });
...
async function saveCheckinTemplate(){const t=collectTemplate(),collapsed=collapsedTemplatePages(),incompleteButton=t.pages.some(p=>p.buttons.some(b=>b.type==="uri"?!b.uri:!b.text));if(!t.entryUrl||!t.pages.length||t.pages.some(p=>!p.imageUrl))return templateStatus("請填寫活動入口網址，並為每一頁上傳圖片或貼上 HTTPS 圖片網址。",false);if(incompleteButton)return templateStatus("請完成按鈕設定：開啟連結需填連結 URL，送出文字需填送出文字。",false);const button=$("#templateSave");button.disabled=true;button.textContent="儲存中...";try{const data=await api("/v1/admin/checkin-template",t);renderCheckinTemplate(data.template||t,collapsed);templateStatus("已儲存模板，已建立新的觀看內容。",true)}catch(error){templateStatus(error.message,false)}finally{button.disabled=false;button.textContent="儲存模板"}}
