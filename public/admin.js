...
function renderCheckinTemplate(template, collapsedPages=null) { const t=normalizeTemplate(template); const collapsed = Array.isArray(collapsedPages) ? collapsedPages : t.pages.map((_, index) => index); checkinTemplateDraft=t; activeCheckinTemplateId=t.id; $("#templateActive").checked=t.active; $("#templateEntryUrl").value=dailyEntryUrl(); $("#templateAltText").value=t.altText; $("#templateRotationMode").value=t.rotationMode; $("#templatePages").innerHTML=t.pages.map((page,index)=>pageHtml(page,index,collapsed.includes(index))).join(""); const select=$("#templateGroupSelect"); if(select){select.innerHTML=checkinTemplates.map((item,index)=>`<option value="${esc(item.id)}" ${item.id===t.id?"selected":""}>${esc(item.altText || `簽到活動 ${index+1}`)}</option>`).join("");} renderTemplatePageToggles(); refreshTemplatePreview(); }
...
async function saveCheckinTemplate(){const t=collectTemplate(),collapsed=collapsedTemplatePages();if(!t.entryUrl||!t.pages.length||t.pages.some(p=>!p.imageUrl))return templateStatus("請填寫活動入口網址，並為每一頁上傳圖片或貼上 HTTPS 圖片網址。",false);const button=$("#templateSave");button.disabled=true;button.textContent="儲存中...";try{const data=await api("/v1/admin/checkin-template",t);checkinTemplates=(data.templates||[]).map(normalizeTemplate);renderCheckinTemplate(data.template||t,collapsed);templateStatus("已儲存簽到活動；可再新增其他組。",true)}catch(error){templateStatus(error.message,false)}finally{button.disabled=false;button.textContent="儲存模板"}}
