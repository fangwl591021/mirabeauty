...
function renderCheckinTemplate(template, collapsedPages=[]) { const t=normalizeTemplate(template); checkinTemplateDraft=t; activeCheckinTemplateId=t.id; $("#templateActive").checked=t.active; $("#templateEntryUrl").value=dailyEntryUrl(); $("#templateAltText").value=t.altText; $("#templateRotationMode").value=t.rotationMode; $("#templatePages").innerHTML=t.pages.map((page,index)=>pageHtml(page,index,collapsedPages.includes(index))).join(""); const select=$("#templateGroupSelect"); if(select){select.innerHTML=checkinTemplates.map((item,index)=>`<option value="${esc(item.id)}" ${item.id===t.id?"selected":""}>${esc(item.altText || `簽到活動 ${index+1}`)}</option>`).join("");} renderTemplatePageToggles(); refreshTemplatePreview(); }
