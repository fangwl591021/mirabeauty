...
async function openMemberDetail(id) {
  ...
  try {
    ...
    panel.querySelectorAll("[data-point-action]").forEach((button) => { button.onclick = async () => { const action = button.dataset.pointAction; const label = {grant:"贈點",deduct:"扣點",backfill:"補登舊點數"}[action]; const raw = prompt(`請輸入${label}數量（正整數）`); if (raw === null) return; const points = Number(raw); if (!Number.isInteger(points) || points <= 0) return showStatus("請輸入正整數點數", "error"); const note = prompt("請輸入操作原因／備註", "") ?? ""; try { await withButtonFeedback(button, async () => { await api(`/v1/admin/members/${encodeURIComponent(id)}/points`, {action,points,note,requestId:crypto.randomUUID()}, "POST"); showStatus(`${label}成功`); await loadMembers(); await openMemberDetail(id); }, {busy:"處理中…",success:"已完成"}); } catch(error) { showStatus(error.message,"error"); } }; });
