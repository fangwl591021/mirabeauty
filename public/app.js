...
function renderLineSourceButtons(buttons, onChange) {
  ...
  holder.innerHTML = buttons.map((button, index) => `<article class="line-source-button" data-card-button-row><div class="line-source-button-head"><strong>按鈕 ${index + 1}</strong><span><button type="button" data-line-toggle>收合</button><button type="button" data-line-remove="${index}">刪除</button></span></div><div class="line-source-button-fields"><label>按鈕顏色<span><input type="color" data-card-button-color-picker value="${esc(button.color || "#B96072")}"><input data-card-button-color value="${esc(button.color || "#B96072")}"></span></label><label>按鈕文字<input data-card-button-label value="${esc(button.label || "")}" placeholder="例如：加入 LINE 好友"></label><label>連結類型<select data-card-button-type><option value="url" ${button.type === "url" ? "selected" : ""}>網址</option><option value="phone" ${button.type === "phone" ? "selected" : ""}>電話</option><option value="email" ${button.type === "email" ? "selected" : ""}>Email</option><option value="line" ${button.type === "line" ? "selected" : ""}>LINE 連結</option><option value="map" ${button.type === "map" ? "selected" : ""}>地圖</option></select></label><label>網址／電話／LINE 連結<input data-card-button-value value="${esc(String(button.value || "").replace(/^(tel:|mailto:)/, ""))}" placeholder="https://... 或 tel:0927..."></label><div class="line-source-button-order"><button type="button" data-line-move="-1" ${index === 0 ? "disabled" : ""}>↑</button><button type="button" data-line-move="1" ${index === buttons.length - 1 ? "disabled" : ""}>↓</button></div></div></article>`).join("");
  const sync = () => onChange?.();
